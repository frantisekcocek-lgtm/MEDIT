...
// Cíl se zadává rychlostí změny váhy. 1 kg tuku ≈ 7700 kcal.
export const CILE = [
  { v: "-0.25", label: "Nabírat 0,25 kg týdně" },
  { v: "0", label: "Udržet váhu" },
  { v: "0.25", label: "Zhubnout 0,25 kg týdně — pomalu a jistě" },
  { v: "0.5", label: "Zhubnout 0,5 kg týdně — doporučené tempo" },
  { v: "0.75", label: "Zhubnout 0,75 kg týdně" },
  { v: "1", label: "Zhubnout 1 kg týdně — jen krátkodobě" },
];

// Přirážka za kojení. Tvorba mléka stojí zhruba 500 kcal denně,
// část z toho si tělo bere ze zásob, proto se běžně doporučuje +330 až +500.
export const KOJENI = [
  { v: "ne", label: "Nekojí", kcal: 0, bilkoviny: 0 },
  { v: "castecne", label: "Kojí částečně (dokrmuje)", kcal: 330, bilkoviny: 15 },
  { v: "plne", label: "Plně kojí", kcal: 500, bilkoviny: 25 },
];

export const KCAL_NA_KG = 7700;

// Nejnižší denní příjem, pod který appka nepustí
function podlaha(p) {
  if (p.kojeni && p.kojeni !== "ne") return 1800;
  return p.pohlavi === "muz" ? 1500 : 1200;
}
...
export function spocitejMakra(p) {
  ...
  const kojeni = KOJENI.find((k) => k.v === (p.kojeni || "ne")) || KOJENI[0];
  const vydej = tdee + kojeni.kcal;

  const pozadovanyUbytek = Number(p.ubytek ?? 0);
  const zadano = Math.round(vydej - (pozadovanyUbytek * KCAL_NA_KG) / 7);

  const min = podlaha(p);
  const kcal = Math.max(min, zadano);
  const omezeno = kcal > zadano;
  const skutecnyUbytek = ((vydej - kcal) * 7) / KCAL_NA_KG;

  const bilkoviny = Math.round(p.vaha * Number(p.bilkovinyNaKg) + kojeni.bilkoviny);
  const tuky = Math.round((kcal * 0.3) / 9);
  const sacharidy = Math.max(0, Math.round((kcal - bilkoviny * 4 - tuky * 9) / 4));

  return {
    tdee,
    kojeniKcal: kojeni.kcal,
    vydej,
    kcal,
    bilkoviny,
    sacharidy,
    ...
    min,
    pozadovanyUbytek,
    skutecnyUbytek: Math.round(skutecnyUbytek * 100) / 100,
  };
}

// Starší uložená nastavení používala procenta — převedeme je na kg za týden
export function migruj(p) {
  if (p.ubytek !== undefined) return { kojeni: "ne", ...p };
  const prevod = { "-20": "1", "-10": "0.5", "0": "0", "10": "-0.25" };
  const { cil, ...zbytek } = p;
  return { ...zbytek, kojeni: "ne", ubytek: prevod[String(cil)] ?? "0.5" };
}
